import { upload, uploadProfileImageToCloudinary } from "../../middleware/uploadToCloudinary.js";
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (existing) return res.status(409).json({ message: "Profile already exists for this userId" });
            ...
            const created = await UserProfile.create({
                ...
                lastName,
                bio,
                profileImage: req.uploadedImageUrl, // ✅ url string (or undefined)
            });
...
router.put(
    ...
    async (req, res) => {
        try {
            ...
            const allowed = ["firstName", "lastName", "bio", "userId"];
            const updates = {};
            ...
            if (req.uploadedImageUrl) updates.profileImage = req.uploadedImageUrl;
